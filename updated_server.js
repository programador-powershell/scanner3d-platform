...
app.post('/api/jobs/:id/stages/:stage/build', async (req, res) => {
  ...
  proc.on('close', async (code) => {
    ...
    if (success) job.build.completed.push(stage);
    await saveJob(job);

    emitJob(id, 'stage_finished', { stage, status: job.stages[stage].status });
    res.json({ success, stage, status: job.stages[stage].status });
  });
});
