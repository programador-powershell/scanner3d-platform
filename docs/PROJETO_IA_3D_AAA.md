...
1. `python training/ingest_knowledge.py` — **TUDO vira aprendizado** (dataset unificado, 400+ exemplos hoje):
   - **`data/references` (or $REFERENCES_DIR) (img + previews 3D)** — visual refs for VLM training. (external D:\References supported via env)
...
Site local em `D:\scanner3d-platform` (project root unificado, MPFB/Anatomy addons copied inside `blender/addons/`). `npm start` → http://localhost:3939. UI única.
...
*Inventário gerado em 2026-06-14T15:14:53.996Z a partir de `D:\scanner3d-platform\data\references`.*

**Total: 0 arquivos — 0 B**
